...
namespace Meta.Tutorial.Framework.Hub.Utilities
{
    ...
    public class DynamicReference
    {
        ...
        {
            return m_referenceType switch
            {
                ReferenceType.SERIALIZED_OBJECT => $"ref:{m_obj.name}",
                ReferenceType.CLASS_TYPE => $"class:{m_classType}|{m_name}",
                ReferenceType.ASSET_PATH => $"path:{m_path}",
                ReferenceType.GUID => $"guid:{m_guid}",
                ReferenceType.INSTANCE_ID => $"instance:{m_instanceID}",
                ReferenceType.SCENE_OBJECT => $"{m_scene.name}::{m_name}",
                _ => null,
            };
        }
